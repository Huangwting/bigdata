...
def get_home_url(page):
    url = 'http://tj.ke.com/ershoufang/pg{}/'.format(page)
    headers = {
...
def save_data(data):
    data_frame = pd.DataFrame(data,
                              columns=['小区位置', '小区名称', '房屋总价（万元）', '房屋单价（元/平米）', '面积(平米)', '户型', '户型结构', '朝向', '装修', '小区均价', '小区建造时间', '小区房屋类型', '小区层数'])

    # 二手房，租房，新房这几个东西（新房 不急，放最后）
    # 字段：小区地点，小区名称、类型（lofter 普通房子什么的）、房型、朝向、楼层、面积、编码、总价、单价、周边、词条（那些介绍的优势 采光好
    # 租房也是包括 租金什么的

    data_frame.to_csv('tianjin_beike_ershoufang.csv', header=False, index=False, mode='a', encoding='utf_8_sig')
